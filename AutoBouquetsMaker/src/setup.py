...
class AutoBouquetsMaker_Setup(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		if config.autobouquetsmaker.level.value == "expert":
			...
			setupList.append(getConfigListEntry(_("Use Custom mode for Sky providers"), config.autobouquetsmaker.custommode, _("Choose whether to apply custom hacks to offcial lists when using Sky providers. Previously, a Custom region had to be selected in the providers menu.")))
